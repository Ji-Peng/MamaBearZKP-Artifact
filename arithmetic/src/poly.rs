...
impl<F: Field> MultiLinearPoly<F> {
    ...
    pub fn eval_multilinear<FEXT: Field<BaseField = F>>(evals: &[F], point: &[FEXT]) -> FEXT {
        let mut scratch = vec![];
        ...
        for r in point[1..].iter() {
            cur_eval_size >>= 1;
            for i in 0..cur_eval_size {
                // `reduce_mod_p` keeps intermediate scratch values within the safe
                // input range for fields with lazy Montgomery arithmetic (e.g. MamaBear).
                // It is a no-op by default for fields whose Add/Sub/Mul always return
                // fully-reduced representatives (e.g. Goldilocks, BabyBear).
                scratch[i] = (scratch[i * 2] + (scratch[i * 2 + 1] - scratch[i * 2]) * (*r)).reduce_mod_p();
            }
        }
        scratch[0]
    }
