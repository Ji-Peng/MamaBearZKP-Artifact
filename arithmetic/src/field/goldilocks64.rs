...
// 2**64 - 2**32 + 1
const MOD: u64 = 18446744069414584321u64;
...
/// Portable fallback for non-x86-64 targets (compile-correctness on aarch64). Same
/// contract: correct only if x + y < 2^64 + ORDER; adds EPSILON on overflow.
#[inline(always)]
#[cfg(not(target_arch = "x86_64"))]
unsafe fn add_no_canonicalize_trashing_input(x: u64, y: u64) -> u64 {
    let (res_wrapped, carry) = x.overflowing_add(y);
    // Add EPSILON == subtract ORDER on overflow.
...
#[cfg(test)]
mod tests {
    use super::{Goldilocks64, MOD};
    use crate::field::Field;

    #[test]
    fn mul_output_is_equal_mod_p() {
        let minus_one = Goldilocks64 { v: MOD - 1 };
        assert!(((minus_one * minus_one) - Goldilocks64::one()).is_zero());
    }
}
