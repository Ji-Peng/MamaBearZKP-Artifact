...
use crate::{prodcheck::ProdEqCheck, sumcheck::Sumcheck};
...
impl<F: Field, PC: PolyCommitProver<F>> Prover<F, PC> {
    pub fn prove(&self, pp: &PC::Param, nv: usize, witness: [Vec<F::BaseField>; 3]) -> Proof {
        ...
        // Gate identity := (S1(X)(L(X)+R(X))+S2(X)(L(X)R(X))-O(X))eq(X,r)
        // S1(x) := 1-S2(x)
        // Gate identity := ((1-S2(X))(L(X)+R(X)) + S2(X)(L(X)R(X)) - O(X))eq(X,r)
        //  := ((1-v[0])*(v[1]+v[2]) + v[0]*v[1]*v[2] + v[3]) * v[4]
