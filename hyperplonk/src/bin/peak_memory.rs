...
#[cfg(target_arch = "x86_64")]
mod x86_impl {
//! Peak memory measurement binary.
//!
//! Measures peak heap-allocated bytes for four phases (`witness_setup`,
//! `pp_setup`, `prove` / `prove_par`, `verify`) of every
//! `hp_df_*` variant. Runs in its own process with
//! `peak_alloc::PeakAlloc` as the global allocator, so it never touches the
//! time-benchmark numbers.
//!
//! Usage:
//!   cargo run --release -p hyperplonk --bin peak_memory -- <variant>
//!
//! Env vars honored:
...
//!                                (key + value + unit) to this txt path.
//!                                pp_setup / prove / prove_par are reported
//!                                in GB; witness_setup / verify in MB
//!                                (both 2 decimals). Stdout always mirrors
//!                                the same line.
...
fn measure_peak<R>(f: impl FnOnce() -> R) -> (R, u64) {
    PEAK.reset_peak_usage();
    let baseline = PEAK.current_usage();
    let out = f();
    let peak = PEAK.peak_usage();
    (out, (peak.saturating_sub(baseline)) as u64)
}

// ---------------------------------------------------------------------------
// Output: append one line per measurement to BENCH_OUTPUT_FILE (txt). The
// stdout mirror always prints the same line. Phase decides the unit:
// pp_setup / prove / prove_par => GB; witness_setup / verify => MB.
// ---------------------------------------------------------------------------
...
impl Recorder {
    ...
    fn put(&mut self, key: String, peak_bytes: u64, phase: &str) {
        let (value, unit) = match phase {
            "pp_setup" | "prove" | "prove_par" => {
                ((peak_bytes as f64) / (1024.0 * 1024.0 * 1024.0), "GiB")
            }
            _ => ((peak_bytes as f64) / (1024.0 * 1024.0), "MiB"),
        };
        let line = format!("{key:<60} {value:>8.2} {unit}");
        println!("{line}");
...
// Each measures witness_setup / pp_setup / prove / verify and records.
// ---------------------------------------------------------------------------

fn run_goldilocks(rec: &mut Recorder, variant: &str, nv: usize, sec: SecurityLevel) {
    let sec_label = sec.label();
    let ((circuit, witness), ws) = measure_peak(|| {
    ...
    rec.put(make_key(variant, Some(sec_label), None, nv, "witness_setup"), ws, "witness_setup");

    let query_num = sec.query_num_goldilocks();
    let ((pp, prover, verifier), pps) = measure_peak(|| {
        ...
        let (pk, vk) =
            circuit.setup::<DeepFoldProver<_>, DeepFoldVerifier<_>>(&pp, &pp);
        (
            pp,
            Prover::<Goldilocks64Ext, DeepFoldProver<Goldilocks64Ext>> { prover_key: pk },
            Verifier::<Goldilocks64Ext, DeepFoldVerifier<Goldilocks64Ext>> { verifier_key: vk },
        )
    });
    rec.put(make_key(variant, Some(sec_label), None, nv, "pp_setup"), pps, "pp_setup");
    drop(circuit);

    let (proof, prove_peak) = measure_peak(|| prover.prove(&pp, nv, witness));
    rec.put(make_key(variant, Some(sec_label), None, nv, "prove"), prove_peak, "prove");

    let (_ok, vpeak) = measure_peak(|| verifier.verify(&pp, nv, proof));
    rec.put(make_key(variant, Some(sec_label), None, nv, "verify"), vpeak, "verify");
}

fn run_goldilocks_sha256(rec: &mut Recorder, variant: &str, nv: usize, sec: SecurityLevel) {
    let sec_label = sec.label();
    let ((circuit, witness, _num_blocks), ws) = measure_peak(|| {
...
fn run_babybear(rec: &mut Recorder, variant: &str, nv: usize, sec: SecurityLevel) {
    ...
    let query_num = sec.query_num_babybear();
    let ((pp, prover, verifier), pps) = measure_peak(|| {
        ...
        (
            pp,
            Prover::<BabyBearExt4, DeepFoldProver<BabyBearExt4>> { prover_key: pk },
            Verifier::<BabyBearExt4, DeepFoldVerifier<BabyBearExt4>> { verifier_key: vk },
        )
    });
    rec.put(make_key(variant, Some(sec_label), None, nv, "pp_setup"), pps, "pp_setup");
    drop(circuit);
...
fn run_mamabear_variant<F: MamaBearExtConfig>(
    ...
    F: Send + Sync,
{
    let ((circuit, witness), ws) = measure_peak(witness_builder);
    rec.put(make_key(variant, Some(label), Some(split), nv, "witness_setup"), ws, "witness_setup");
    ...
    let ((pp, prover, verifier), pps) = measure_peak(|| {
        ...
        (
            pp,
            ProverMamaBear::<F> { prover_key: pk },
            VerifierMamaBear::<F> { verifier_key: vk },
        )
    });
    rec.put(make_key(variant, Some(label), Some(split), nv, "pp_setup"), pps, "pp_setup");
    drop(circuit);

    let prove_phase = if parallel { "prove_par" } else { "prove" };
    let (proof, prove_peak) = measure_peak(|| {
        if parallel {
    ...
    rec.put(
        make_key(variant, Some(label), Some(split), nv, prove_phase),
        prove_peak,
        prove_phase,
    );

    let (_ok, vpeak) = measure_peak(|| verifier.verify(&pp, nv, proof));
    rec.put(make_key(variant, Some(label), Some(split), nv, "verify"), vpeak, "verify");
}
...
pub fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: peak_memory <variant>");
