//! deepfold_goldilocks: custom warmup+median harness (replaces criterion).
//!
//! Env vars:
...
// Case holds raw polynomial data only. Provers (prover_key_commitments,
// witness_pc) are built lazily inside bench_open to avoid OOM at NV=24.
struct Case {
    nv: usize,
    pp: DeepFoldParam<Goldilocks64Ext>,
    fixed: Vec<Vec<Goldilocks64>>,
    witness: Vec<Vec<Goldilocks64>>,
    point: Vec<Goldilocks64Ext>,
...
fn build_case(nv: usize, query_num: usize) -> Case {
    ...
    let len = 1usize << nv;
    fn rand_poly(rng: &mut SmallRng, len: usize) -> Vec<Goldilocks64> {
        (0..len).map(|_| Goldilocks64::random(&mut *rng)).collect()
    }

    let selector = rand_poly(&mut rng, len);
    let permutation: Vec<Vec<Goldilocks64>> =
        (0..3).map(|_| rand_poly(&mut rng, len)).collect();
    let witness: Vec<Vec<Goldilocks64>> =
    ...
    let fixed: Vec<Vec<Goldilocks64>> = std::iter::once(selector)
...
fn bench_case(case: &Case, sec_label: &str) {
    measure(
        &format!("Goldilocks {sec_label} Commit NV={}", case.nv),
        || {
            let witness = case.witness.clone();
            let start = Instant::now();
            let prover = DeepFoldProver::<Goldilocks64Ext>::new(&case.pp, &witness);
    ...
    // Build provers locally for the open bench so they don't coexist with the
    // commit-bench transients (which would push peak RAM past 60 GB at NV=24).
    let prover_key_commitments = DeepFoldProver::<Goldilocks64Ext>::new(&case.pp, &case.fixed);
    let witness_pc = DeepFoldProver::<Goldilocks64Ext>::new(&case.pp, &case.witness);

    measure(
        &format!("Goldilocks {sec_label} Open NV={}", case.nv),
        || {
            let mut transcript = Transcript::new();
            let start = Instant::now();
            DeepFoldProver::<Goldilocks64Ext>::open(
                &case.pp,
                vec![&prover_key_commitments, &witness_pc],
                case.point.clone(),
                &mut transcript,
            );
            black_box(transcript);
