//! deepfold_mamabear: custom warmup+median harness (replaces criterion).
//!
//! Measures Commit and Open as separate cells. HyperPlonk-realistic working
//! set: commit takes 3 witness polynomials, open takes the 4-poly
//! prover_key.commitments (selector + 3 permutations) plus the 3-poly
//! witness_pc.
//!
//! Env vars:
...
struct Case<F: DeepFoldExtField> {
    nv: usize,
    split: usize,
    ...
    FRand: FnMut(&mut SmallRng) -> F,
{
    ...
    let fixed_refs: Vec<&[MamaBearScalar]> = std::iter::once(selector.as_slice())
        ...
        .collect();
    let prover_key_commitments = DeepFoldMamaBearProver::<F>::new(&pp, &fixed_refs);

    let witness_refs: Vec<&[MamaBearScalar]> = witness.iter().map(|w| w.as_slice()).collect();
    let witness_pc = DeepFoldMamaBearProver::<F>::new(&pp, &witness_refs);
...
fn bench_commit<F: DeepFoldExtField>(case: &Case<F>, ext_label: &str, sec_label: &str) {
    measure(
        &format!(
            "{ext_label} {sec_label} Commit NV={} split={}",
            case.nv, case.split
        ),
        || {
            ...
            let prover = DeepFoldMamaBearProver::<F>::new(&case.pp, &witness_refs);
...
fn bench_open<F: DeepFoldExtField>(case: &Case<F>, ext_label: &str, sec_label: &str) {
    measure(
        &format!(
            "{ext_label} {sec_label} Open NV={} split={}",
            case.nv, case.split
        ),
        || {
            let mut transcript = Transcript::new();
            let start = Instant::now();
            DeepFoldMamaBearProver::<F>::open(
                &case.pp,
    ...
    for nv in nv_min..=nv_max {
        match sec {
            ...
            SecurityLevel::Prov128 => {
                let case = build_case::<MamaBearScalarExt3, _>(
                    nv,
                    split,
                    sec,
                ...
                bench_commit(&case, "Ext3", sec_label);
                bench_open(&case, "Ext3", sec_label);
                drop(case);
            }
        }
    }
