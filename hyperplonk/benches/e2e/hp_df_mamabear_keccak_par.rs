//! hp_df_mamabear_keccak_par: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (parallel prover) on a Keccak-f permutation circuit.
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_keccak_par] skip NV={nv}: \
