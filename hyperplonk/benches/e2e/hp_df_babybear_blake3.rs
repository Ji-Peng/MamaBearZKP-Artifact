//! hp_df_babybear_blake3: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold on BabyBear with a BLAKE3 permutation circuit.
...
//! plonky3's `--log-trace-length = log2(num_perms)` row. This BabyBear
//! variant is the direct counterpart of plonky3's `--field baby-bear
//! --objective blake-3-permutations` sweep driven by `reproduce_plonky3.sh`.
//!
//! Env vars:
...
use hyperplonk::{
    blake3_circuit::build_blake3_circuit_babybear, circuit::Circuit, prover::Prover,
    verifier::Verifier,
};
...
fn build_case(nv: usize, query_num: usize) -> BenchCase {
    ...
    let (circuit, witness): (Circuit<BabyBearExt4>, _) =
        build_blake3_circuit_babybear(num_perms, nv);
...
fn bench_case(case: &BenchCase, sec_label: &str) {
    measure(
        &format!(
            "HP DF BabyBear BLAKE3 {sec_label} prove NV={} perms={}",
    ...
    measure(
        &format!(
            "HP DF BabyBear BLAKE3 {sec_label} verify NV={} perms={}",
            case.nv, case.num_perms
        ),
        || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_babybear_blake3] skip NV={nv}: \
