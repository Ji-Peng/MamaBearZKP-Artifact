//! hp_df_mamabear_blake3: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (serial prover) on a BLAKE3 permutation circuit.
...
//!
//! NVs below `min_nv_for(BLAKE3_GATES_PER_PERMUTATION)` (~18) are skipped.
...
fn bench_case<F: MamaBearExtConfig>(label: &str, sec_label: &str, case: &BenchCase<F>) {
    measure(
        &format!(
            "HP DF MamaBear BLAKE3 {label} split={} {sec_label} prove NV={} perms={}",
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear BLAKE3 {label} split={} {sec_label} verify NV={} perms={}",
                case.pp.split_level, case.nv, case.num_perms
            ),
            || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_blake3] skip NV={nv}: \
