//! hp_df_mamabear_par: custom warmup+median harness (replaces criterion).
//!
//! Random-circuit HyperPlonk + DeepFold (parallel prover).
//!
//! Env vars:
    ...
    F: Send + Sync,
{
    measure(
        &format!(
            "HP DF MamaBear Par {label} split={} {sec_label} prove_par NV={}",
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear Par {label} split={} {sec_label} verify_par NV={}",
                case.pp.split_level, case.nv
            ),
            || {
