//! hp_df_mamabear_aes128_par: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (parallel prover) on an AES-128 circuit.
//!
//! Env vars:
    ...
    F: Send + Sync,
{
    measure(
        &format!(
            "HP DF MamaBear AES128 Par {label} split={} {sec_label} prove_par NV={} calls={}",
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear AES128 Par {label} split={} {sec_label} verify_par NV={} calls={}",
                case.pp.split_level, case.nv, case.num_calls
            ),
            || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_aes128_par] skip NV={nv}: \
