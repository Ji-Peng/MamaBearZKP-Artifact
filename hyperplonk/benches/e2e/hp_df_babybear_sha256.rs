//! hp_df_babybear_sha256: custom warmup+median harness (replaces criterion).
//!
//! Env vars:
...
use hyperplonk::{
    circuit::Circuit, prover::Prover, sha256_circuit::build_sha256_circuit_babybear,
    verifier::Verifier,
};
...
fn bench_case(case: &BenchCase, sec_label: &str) {
    measure(
        &format!(
            "HP DF BabyBear SHA256 {sec_label} prove NV={} blocks={}",
    ...
    measure(
        &format!(
            "HP DF BabyBear SHA256 {sec_label} verify NV={} blocks={}",
            case.nv, case.num_blocks
        ),
        || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_babybear_sha256] skip NV={nv}: \
