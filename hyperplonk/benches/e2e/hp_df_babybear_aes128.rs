//! hp_df_babybear_aes128: custom warmup+median harness (replaces criterion).
//!
//! Env vars:
...
use hyperplonk::{
    aes_circuit::build_aes128_circuit_babybear, circuit::Circuit, prover::Prover,
    verifier::Verifier,
};
...
fn bench_case(case: &BenchCase, sec_label: &str) {
    measure(
        &format!(
            "HP DF BabyBear AES128 {sec_label} prove NV={} calls={}",
    ...
    measure(
        &format!(
            "HP DF BabyBear AES128 {sec_label} verify NV={} calls={}",
            case.nv, case.num_calls
        ),
        || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_babybear_aes128] skip NV={nv}: \
