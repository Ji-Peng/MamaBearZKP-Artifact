//! hp_df_mamabear_sha256: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (serial prover) on a SHA-256 circuit.
//!
//! Env vars:
...
//!
//! NVs below `min_nv_for(SHA256_GATES_PER_BLOCK)` (~19) are skipped.
...
use hyperplonk::{
    ...
    sha256_circuit::build_sha256_circuit,
...
fn bench_case<F: MamaBearExtConfig>(label: &str, sec_label: &str, case: &BenchCase<F>) {
    measure(
        &format!(
            "HP DF MamaBear SHA256 {label} split={} {sec_label} prove NV={} blocks={}",
            case.pp.split_level, case.nv, case.num_blocks
        ),
        || {
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear SHA256 {label} split={} {sec_label} verify NV={} blocks={}",
                case.pp.split_level, case.nv, case.num_blocks
            ),
            || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_sha256] skip NV={nv}: \
                 circuit needs >= 2^{min_nv} gates (1 SHA-256 block)"
            );
            continue;
        }
        match sec {
