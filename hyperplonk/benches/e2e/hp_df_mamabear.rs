//! hp_df_mamabear: custom warmup+median harness (replaces criterion).
//!
//! Random-circuit HyperPlonk + DeepFold (serial prover).
//!
//! Env vars:
...
fn bench_case<F: MamaBearExtConfig>(label: &str, sec_label: &str, case: &BenchCase<F>) {
    measure(
        &format!(
            "HP DF MamaBear {label} split={} {sec_label} prove NV={}",
            case.pp.split_level, case.nv
        ),
        || {
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear {label} split={} {sec_label} verify NV={}",
                case.pp.split_level, case.nv
            ),
            || {
