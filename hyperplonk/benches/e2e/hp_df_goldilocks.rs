//! hp_df_goldilocks: custom warmup+median harness (replaces criterion).
//!
//! Random-circuit HyperPlonk + DeepFold (Goldilocks).
//!
//! Env vars:
...
fn bench_case(case: &BenchCase, sec_label: &str) {
    measure(
        &format!("HP DF Goldilocks {sec_label} prove NV={}", case.nv),
        || {
    ...
    measure(
        &format!("HP DF Goldilocks {sec_label} verify NV={}", case.nv),
        || {
