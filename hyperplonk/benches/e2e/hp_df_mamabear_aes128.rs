//! hp_df_mamabear_aes128: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (serial prover) on an AES-128 circuit.
//!
//! Env vars:
...
use hyperplonk::{
    aes_circuit::build_aes128_circuit,
...
fn build_case<F: MamaBearExtConfig>(
    ...
    let (circuit, witness_raw) = build_aes128_circuit::<F>(num_calls, nv);
...
fn bench_case<F: MamaBearExtConfig>(label: &str, sec_label: &str, case: &BenchCase<F>) {
    measure(
        &format!(
            "HP DF MamaBear AES128 {label} split={} {sec_label} prove NV={} calls={}",
            case.pp.split_level, case.nv, case.num_calls
        ),
        || {
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear AES128 {label} split={} {sec_label} verify NV={} calls={}",
                case.pp.split_level, case.nv, case.num_calls
            ),
            || {
...
fn main() {
    ...
    let min_nv = min_nv_for(AES128_GATES_PER_CALL);
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_aes128] skip NV={nv}: \
                 circuit needs >= 2^{min_nv} gates (1 AES-128 call)"
            );
            continue;
        }
        match sec {
