//! hp_df_mamabear_sha256_par: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (parallel prover) on a SHA-256 circuit.
//!
//! Env vars:
    ...
    F: Send + Sync,
{
    measure(
        &format!(
            "HP DF MamaBear SHA256 Par {label} split={} {sec_label} prove_par NV={} blocks={}",
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear SHA256 Par {label} split={} {sec_label} verify_par NV={} blocks={}",
                case.pp.split_level, case.nv, case.num_blocks
            ),
            || {
...
fn main() {
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_sha256_par] skip NV={nv}: \
