//! hp_df_mamabear_keccak: custom warmup+median harness (replaces criterion).
//!
//! HyperPlonk + DeepFold (serial prover) on a Keccak-f permutation circuit.
//! The permutation unit matches plonky3's `p3-blake3-air::Blake3Air` (7 rounds,
//! no feed-forward). At NV=X this bench proves
//! `floor(2^X / KECCAKF_GATES_PER_PERMUTATION)` permutations, comparable to
...
//!
//! NVs below `min_nv_for(KECCAKF_GATES_PER_PERMUTATION)` (~18) are skipped.
...
use hyperplonk::{
    keccakf_circuit::build_keccakf_circuit,
...
fn build_case<F: MamaBearExtConfig>(
    ...
    let num_perms = (total_gates / KECCAKF_GATES_PER_PERMUTATION).max(1);

    let (circuit, witness_raw) = build_keccakf_circuit::<F>(num_perms, nv);
    ...
    let verify_ok = verifier.verify(&pp, nv, proof.clone());
...
fn bench_case<F: MamaBearExtConfig>(label: &str, sec_label: &str, case: &BenchCase<F>) {
    measure(
        &format!(
            "HP DF MamaBear Keccak {label} split={} {sec_label} prove NV={} perms={}",
    ...
    if case.verify_ok {
        measure(
            &format!(
                "HP DF MamaBear Keccak {label} split={} {sec_label} verify NV={} perms={}",
                case.pp.split_level, case.nv, case.num_perms
            ),
            || {
...
fn main() {
    ...
    let min_nv = min_nv_for(KECCAKF_GATES_PER_PERMUTATION);
    ...
    for nv in nv_min..=nv_max {
        if nv < min_nv {
            eprintln!(
                "[hp_df_mamabear_keccak] skip NV={nv}: \
                 circuit needs >= 2^{min_nv} gates (1 Keccak-f permutation)"
            );
            continue;
        }
        match sec {
