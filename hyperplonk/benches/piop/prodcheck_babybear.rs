//! prodcheck_babybear: custom warmup+median harness (replaces criterion).
//!
//! Env vars:
...
use arithmetic::field::babybear::{BabyBearExt4, BabyBearField};
...
fn bench_prodcheck(nv: usize) {
    ...
    let bookkeeping: [Vec<BabyBearExt4>; 3] =
        std::array::from_fn(|_| random_evals(&mut rng, domain));
    let permutation_polys: [Vec<BabyBearField>; 3] =
        std::array::from_fn(|_| random_evals(&mut rng, domain));

    measure(&format!("ProdEqCheck BabyBear NV={nv}"), || {
        let mut t = Transcript::new();
        let start = Instant::now();
        let witness_flatten: Vec<BabyBearExt4> = bookkeeping[0]
            .clone()
            .into_iter()
            .chain(bookkeeping[1].clone().into_iter())
            .chain(bookkeeping[2].clone().into_iter())
            .chain((0..domain).map(|_| BabyBearExt4::zero()))
            .collect();
        let identical: Vec<BabyBearField> =
            MultiLinearPoly::new_identical(nv, BabyBearField::zero())
                .evals
                .into_iter()
                .chain(
                    MultiLinearPoly::new_identical(nv, BabyBearField::from(1u64 << 29))
                        .evals
                        .into_iter(),
                )
                .chain(
                    MultiLinearPoly::new_identical(nv, BabyBearField::from(1u64 << 30))
                        .evals
                        .into_iter(),
                )
                .chain((0..domain).map(|_| BabyBearField::zero()))
                .collect();
        let perm_flat: Vec<BabyBearField> = permutation_polys[0]
            .clone()
            .into_iter()
            ...
            .chain((0..domain).map(|_| BabyBearField::zero()))
            .collect();
        let r = [0; 2].map(|_| t.challenge_f::<BabyBearExt4>());
