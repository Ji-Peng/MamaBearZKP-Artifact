//! zerocheck_babybear: custom warmup+median harness (replaces criterion).
//!
//! Env vars:
...
fn bench_zerocheck(nv: usize) {
    ...
    let selector_base: Vec<BabyBearField> = random_evals(&mut rng, domain_size);
    let bookkeeping: [Vec<BabyBearExt4>; 3] =
        std::array::from_fn(|_| random_evals(&mut rng, domain_size));
    let r: Vec<BabyBearExt4> = (0..nv).map(|_| BabyBearExt4::random(&mut rng)).collect();

    measure(&format!("Zerocheck BabyBearExt4 NV={nv}"), || {
